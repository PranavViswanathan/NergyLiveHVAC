                ...
                if is_valid_url(url=config.READ_BACNET_DATA_URL):
                    ...
                    # Check the response status
                        if response.getcode() == 200:
                            ...
                            logging.error(f"Failed to send JSON data. Status code: {response.getcode()}", exc_info=True)
            ...
            try:
                ...
                with urllib.request.urlopen(request2) as response:
                    ...
                    if response.getcode() == 200:
                        ...
                        print(f"Failed to send JSON data. Status code: {response.getcode()}")
                        logging.error(f"Failed to send JSON data. WRITE_BACNETAPI Status code: {response.getcode()}", exc_info=True)
            except Exception as e:
                print(f"An error occurred: {e}")
                logging.error(f"An error occuredWRITE_BACNETAPI. : {e}", exc_info=True)
                ...
                try:
                    ...
                    with urllib.request.urlopen(request2) as response:
                        ...
                        if response.getcode() == 200:
                            ...
                            print(f"Failed to send JSON data. Status code: {response.getcode()}")
                            logging.error(f"Failed to send JSON data. WRITE_ACK_API Status code: {response.getcode()}", exc_info=True)
                except Exception as e:
                    print(f"An error occurred during API3 request: {e}")
                    logging.error(f"An error occured.WRITE_ACK_API : {e}", exc_info=True)

except Exception as e:
    print(f"An error occurred: {e}")
    logging.error(f"An error occured.COMMON : {e}", exc_info=True)
